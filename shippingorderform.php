...
for($num = 0; $num < count($importer_data); $num ++){
    ...
    $query = "insert into shipping_order (containerId,sourceHarborId,destinationHarborId,FFCId,shipID,onShipDate,arrivalDate,arrived,completedByTruck,orderDate,importer_orders_in_container) values ('{$containerId}', '{$sourceHarborId}', '{$destinationHarborId}', '{$ffcId}', '{$ship_id}', '{$onShipDate}', '{$arrivalDate}', 0,0,CURDATE(),'{$fin_data}')";
  ...
  <nav class="navbar navbar-expand-lg bg-light">
    ...
    <script>
        ...
        function setJson()
        {
            ...
            var m = month +1;
            var date = m+"-"+currentdate.getDate().toString()+"-"+currentdate.getFullYear().toString();
